...
-   Develop targeted interventions, such as promoting indoor air filtration on high-pollution days [@maestas2018].
...
You can querry the V_TRI_FORM_R\_EZ using the API even though it's not a documented table listed in the API website. This form contains facility information and AIR_TOTAL_RELEASE reporting, but it only goes until 2021. To get the most recent data (2023) you would have to manually download the individual tables with separate calls perform the joins, or use the [custom form search web portal](https://enviro.epa.gov/facts/tri/form_ra_download.html). We'll demonstrate a quick API call for the V_TRI_FORM_R\_EZ, but move forward with the most recent data available in the custom form search web portal.
